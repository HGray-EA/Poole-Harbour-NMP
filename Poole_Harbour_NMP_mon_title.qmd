...
# Load in CAT data and template for leaflet plots
...
```{r, Freshwater Mon Sites}

# Get monitoring sites from the Harbour

options(timeout = 1000) # set timeout to longer than default as APIs timeout.
...
# You can unhash this code should you want to pull the most up-to-date WIMS data, it takes a while as for all of Wessex area.
# You would then have to hash the below read.csv
...
wd <- read.csv("/dbfs/FileStore/WSX_HGray/9943_WIMS.csv")

  Transform.api <- function(df){
    
           df %>% 
                 # Change class of incoming data
                 mutate(
                 ...
                 # Change type of data from df to sf  
...
# Mon Sites Continuation

# Certain 

  certain <- c("SW-50590127", "SW-50450129")
    RSN <- c("SW-RSN0029","SW-RSN0397","SW-RSN0621","SW-RSN0653","SW-RSN1197","SW-RSN1685","SW-RSN2045","SW-RSN2213")
  Until.2026 <- c("SW-C0538000", "SW-50900712", "SW-50951010")
  
  Nomore <-       c("SW-50410164", "SW-50530197", "SW-50450320")
  
  Riv.dat <- read.csv("/dbfs/FileStore/WSX_HGray/0116_WIMS.csv") %>% 
                 st_as_sf(., coords=c("sample.purpose.label","sample.samplingPoint.easting"), crs= 27700) %>% 
                  st_transform(4326)
  
  C_Riv.dat <- Riv.dat %>% filter(sample.samplingPoint.notation %in% certain) 
  RSN_Riv.dat <- Riv.dat %>% filter(sample.samplingPoint.notation %in% RSN) 
  TW_Riv.dat <- Riv.dat %>% filter(sample.samplingPoint.notation %in% Until.2026)
  No_Riv.dat <- Riv.dat %>% filter(sample.samplingPoint.notation %in% Nomore) 
  
# Transform api data
  Riv.dat<- Transform.api(wd.land)
  
# Crop data to catchment of interest
  Riv.dat <- Riv.dat[CAT,]
...
### Total Marine N in Poole Harbour by Site
This visual displays the same data as above but non-spatially.
...
For each pair of plots a tab for rolling mean plots is available, use this to identify more granular trends in the data.
...
# GAM
  gam_PO <- gam(Result ~ s(as.numeric(Date), bs = "cs"), data = FR.O.wq.data, gamma = 0.5)

#Predict
# We don't use this anymore but we can predict the future.
...
# Fit the GAM to existing data  
  FR.O.wq.data$fitted_values <- predict(gam_PO)
  
# Create a variable for all subsequent plots of Orthophosphate
    Ortho_gg <- geom_point(alpha = 0.2, col = "#c24f00") +
...
# GAM for Nitrogen
gam_FN <- gam(Result ~ s(as.numeric(Date), bs = "cs"), data = FR.TotN.wq.data, gamma = 1)



#Predict
FR.TotN.wq.data$fitted_values <- predict(gam_FN)
...
FB <- ggplot(FR.TotN.wq.data, aes(x = Date, y = Result)) +
  ...
  geom_rug(sides = "l", alpha = 0.2) +  
  geom_line(aes(y=fitted_values), col="seagreen")+
#  stat_smooth(method = "gam", formula = y ~ s(x, bs = "cs"), se = TRUE, col="seagreen")+
  ylim(c(0,8))+
 # geom_line(data = future_dates, aes(x = Date, y = Predicted_Result), col = "darkred", alpha = 0.8, linetype = "dashed") +
...
A rolling mean was applied instead of a GAM to provide more resolution to the average of the data. visual assessment and afc assessment were made to identify the n number of best fit to calculate the mean from a given rolling time period; 3 month clusters were chosen to capture seasonal changes in hydrology. 
```{r, Rolling Mean Plots Frome}

  
# GAM Okay but stakeholders want more resolution.  
# Define rolling mean function
      ...
      # Apply rolling mean to the 'Result' column
      FR.O.wq.data$rolling_mean <- ma(FR.O.wq.data$Result, n)
      
 Roll_FA <-  ggplot(FR.O.wq.data, aes(x = Date, y = Result)) +
        geom_point(alpha = 0.2, col = "#c24f00") +
        geom_rug(sides = "l", alpha = 0.2) +  
      # ylim(c(0,0.06))+
        geom_line(aes(y = rolling_mean), col = "seagreen", alpha = 0.8, linetype = "solid") +
        labs(title = "Frome, Orthophosphate",
             x = "Date",
             y = "Orthophosphate (mg/L)")+
        theme_bw()
 ...
 # Total Nitrogen
      ...
      FR.TotN.wq.data$rolling_mean <- ma(FR.TotN.wq.data$Result, n)
 
  Roll_FB <-  ggplot(FR.TotN.wq.data, aes(x = Date, y = Result)) +
        geom_point(alpha = 0.2, col = "#0073C2FF") +
        geom_rug(sides = "l", alpha = 0.2) +  
        ylim(c(0,8))+
        geom_line(aes(y = rolling_mean), col = "seagreen", alpha = 0.8, linetype = "solid") +
...
```
#### Rolling Mean Plots
```{r, Rolling Mean Plot Piddle}


      # Apply rolling mean to the 'Result' column
      P.O.wq.data$rolling_mean <- ma(P.O.wq.data$Result, n)
      
 Roll_PA <-  ggplot(P.O.wq.data, aes(x = Date, y = Result)) +
        geom_point(alpha = 0.2, col = "#c24f00") +
        geom_rug(sides = "l", alpha = 0.2) +  
      # ylim(c(0,0.06))+
        geom_line(aes(y = rolling_mean), col = "seagreen", alpha = 0.8, linetype = "solid") +
 ...
 # Total Nitrogen
      ...
      P.TotN.wq.data$rolling_mean <- ma(P.TotN.wq.data$Result, n)
 
  Roll_PB <-  ggplot(P.TotN.wq.data, aes(x = Date, y = Result)) +
        geom_point(alpha = 0.2, col = "#0073C2FF") +
        geom_rug(sides = "l", alpha = 0.2) +  
        ylim(c(0,8.6))+
        geom_line(aes(y = rolling_mean), col = "seagreen", alpha = 0.8, linetype = "solid") +
...
```
#### Rolling Mean Plots
```{r, Rolling Mean Plot Sherford}


      # Apply rolling mean to the 'Result' column
      Sher.O.wq.data$rolling_mean <- ma(Sher.O.wq.data$Result, n)
      
 Roll_SA <-  ggplot(Sher.O.wq.data, aes(x = Date, y = Result)) +
        geom_point(alpha = 0.2, col = "#c24f00") +
        geom_rug(sides = "l", alpha = 0.2) +  
      # ylim(c(0,0.06))+
        geom_line(aes(y = rolling_mean), col = "seagreen", alpha = 0.8, linetype = "solid") +
 ...
 # Total Nitrogen
      ...
      Sher.N.wq.data$rolling_mean <- ma(Sher.N.wq.data$Result, n)
 
  Roll_SB <-  ggplot(Sher.N.wq.data, aes(x = Date, y = Result)) +
        geom_point(alpha = 0.2, col = "#0073C2FF") +
        geom_rug(sides = "l", alpha = 0.2) +  
        ylim(c(0,8))+
        geom_line(aes(y = rolling_mean), col = "seagreen", alpha = 0.8, linetype = "solid") +
...
```
#### Rolling Mean Plots
```{r, Rolling Mean Plot Corfe}

      # Apply rolling mean to the 'Result' column
      C.O.wq.data$rolling_mean <- ma(C.O.wq.data$Result, n)
      
 Roll_CA <-  ggplot(C.O.wq.data, aes(x = Date, y = Result)) +
        geom_point(alpha = 0.2, col = "#c24f00") +
        geom_rug(sides = "l", alpha = 0.2) +  
      # ylim(c(0,0.06))+
        geom_line(aes(y = rolling_mean), col = "seagreen", alpha = 0.8, linetype = "solid") +
 ...
 # Total Nitrogen
      ...
      C.TotN.wq.data$rolling_mean <- ma(C.TotN.wq.data$Result, n)
 
  Roll_CB <-  ggplot(C.TotN.wq.data, aes(x = Date, y = Result)) +
        geom_point(alpha = 0.2, col = "#0073C2FF") +
        geom_rug(sides = "l", alpha = 0.2) +  
        ylim(c(0,8))+
        geom_line(aes(y = rolling_mean), col = "seagreen", alpha = 0.8, linetype = "solid") +
